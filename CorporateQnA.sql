...
SELECT qdat.QuestionId, qdat.CategoryId, u.Name, qdat.Title as 'QuestionTitle', qdat.Content, qdat.AskedBy, qdat.AskedOn, qdat.LikeCount, qdat.ViewCount, qdat.Resolved, qdat.AnswerCount  FROM 
(SELECT q.Id as 'QuestionId', q.Title, q.Content, q.CategoryId,q.AskedOn , q.AskedBy, qac.AnswerCount ,
	...
	(Select Count(*) FROM QuestionActivity qa Where q.Id = qa.QuestionId AND qa.ActivityType = 1) as 'ViewCount', 
	(Select Count(*) FROM QuestionActivity qa Where q.Id = qa.QuestionId AND qa.ActivityType = 2) as 'Resolved' 
FROM Question q
INNER JOIN
